...
function getPendingThesesData() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let sheet = ss.getSheetByName("Theses"); // ดึงจากชีทชื่อ Theses
    ...
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      if (row.length < 11) continue; // ข้ามแถวที่คอลัมน์ไม่ครบ
     
      const status = row[10] ? String(row[10]).trim() : ""; // คอลัมน์ K (Status)
     
      if (status === "รอตรวจสอบ") {
        pendingList.push({
          rowId: row[0] ? String(row[0]).trim() : "",        // คอลัมน์ A (ID)
          author: row[2] ? String(row[2]).trim() : "ไม่ระบุผู้แต่ง", // คอลัมน์ C (Author)
          degree: row[3] ? String(row[3]).trim() : "ไม่ระบุ", // คอลัมน์ D (Degree)
          titleTh: row[4] ? String(row[4]).trim() : "ไม่มีชื่อภาษาไทย", // คอลัมน์ E (Title TH)
          titleEn: row[5] ? String(row[5]).trim() : "",       // คอลัมน์ F (Title ENG)
          major: row[6] ? String(row[6]).trim() : "ไม่ระบุสาขา", // 👈 คอลัมน์ G (Department / สาขาวิชา)
          year: row[7] ? String(row[7]).trim() : "",         // คอลัมน์ H (Year)
          advisor: row[8] ? String(row[8]).trim() : "",      // คอลัมน์ I (Advisor)
          pdfUrl: row[9] ? String(row[9]).trim() : "#"        // คอลัมน์ J (PDF_URL)
        });
      }
    }
    return pendingList;
